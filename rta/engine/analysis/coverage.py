...
def parse_sdc_coverage(text: str, filename: str = "") -> CoverageResult:
    ...
    The raw text is normalized through the shared preprocessor first so that
    commented-out constraints, backslash continuations, and Tcl variables are
    interpreted the same way as the checker/converter/clock relations (Phase 4
    consistency fix: comments must not create phantom 'present' items).
    """
    # Normalize via shared preprocessor: strip comments, join continuations,
    # resolve bounded Tcl variables — then analyze the logical text.
