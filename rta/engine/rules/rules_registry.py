...
# ── Semantic validation (SDC-046..049) — Phase 5 ──────────────────────────────

_r("SDC-046", "error", "Undefined Clock in I/O Delay",
   "A set_input_delay / set_output_delay -clock names a clock that no create_clock or create_generated_clock defines.",
   "Real tools fail or warn on this because the port-to-clock association is broken — the delay would be applied to nothing, and static analysis must NOT silently substitute a different clock.",
   "Fix the -clock name, or add the missing create_clock / create_generated_clock definition.",
   "https://www.synopsys.com/glossary/what-is-sdc.html", "checker", "1.4.0")

_r("SDC-047", "warning", "Undefined Master Clock",
   "A create_generated_clock -master_clock names a clock that is not defined in this SDC.",
   "Without a resolvable master clock the generated clock's period and phase are undefined, making downstream timing meaningless.",
   "Add the create_clock/create_generated_clock that defines the master, or fix the -master_clock name.",
   "https://www.synopsys.com/glossary/what-is-sdc.html", "checker", "1.4.0")

_r("SDC-048", "warning", "Undefined Clock in Clock Groups",
   "A set_clock_groups -group names a clock that is not defined by any create_clock/create_generated_clock.",
   "Grouping a nonexistent clock silently excludes nothing — CDC paths involving it remain unhandled.",
   "Fix the clock name, or define the missing clock first.",
   "https://www.ausdia.com/blog/5/seemingly-simple-clock-relations-quiz/filter/0", "checker", "1.4.0")

_r("SDC-049", "warning", "Contradictory Case Analysis",
   "The same pin/port is assigned two different set_case_analysis values in one file (e.g. 0 then 1).",
   "A pin cannot be both 0 and 1 in the same mode — the later value silently overrides the earlier, hiding a real conflict.",
   "Use one value per object per mode, or split the modes into separate SDC files.",
   "", "checker", "1.4.0")
...
# ── Design-aware validation (SDC-055..059) — Phase 8 ─────────────────────────
# These rules fire ONLY when an optional design context (Verilog netlist or
# object inventory) is supplied alongside the SDC. In SDC-only mode they are
# never produced — the validator stays silent where it cannot prove anything.

_r("SDC-055", "error", "Design Object Not Found",
   "A get_ports/get_pins/get_cells/get_nets reference names an explicit object that does not exist in the supplied design context.",
   "Referencing a nonexistent object makes the constraint silently apply to nothing — real timing paths stay unconstrained while the SDC looks correct.",
   "Fix the object name, or add the missing port/pin/cell/net to the design.",
   "", "design_context", "1.4.0")

_r("SDC-056", "warning", "Empty Collection",
   "A wildcard collection (e.g. [get_ports debug_*]) matched nothing in the supplied design context.",
   "An empty collection means the constraint applies to zero objects — a typo'd or stale pattern quietly leaves real objects unconstrained.",
   "Verify the wildcard pattern matches at least one design object.",
   "", "design_context", "1.4.0")

_r("SDC-057", "error", "Invalid Hierarchy Reference",
   "A get_pins/get_cells path references an instance parent that does not exist in the design hierarchy (e.g. u_core/u_nope/D where u_core/u_nope is absent).",
   "Invalid hierarchy paths resolve to nothing in the tool — the intended pins/cells are never constrained.",
   "Fix the hierarchical path to match a real instance in the netlist.",
   "", "design_context", "1.4.0")

_r("SDC-058", "error", "Top Module Ambiguity",
   "The supplied netlist has multiple candidate top modules (or the requested top does not exist) — design context could not be built.",
   "Without a resolved top module, port-level and hierarchy validation cannot run; ambiguous selection would silently pick the wrong design boundary.",
   "Specify the top module explicitly (--top / UI selector), or add a unique top module.",
   "", "design_context", "1.4.0")

_r("SDC-059", "warning", "Unconstrained Port",
   "A data-pattern input/output port of the supplied design has no set_input_delay/set_output_delay (and is not a clock port).",
   "Unconstrained boundary ports leave real interface timing unverified — the design can pass synthesis timing while failing at the chip boundary.",
   "Add set_input_delay/set_output_delay for the port, or explicitly document why it is exempt (clock/reset/control).",
   "", "design_context", "1.4.0")

# ── Design constraint coverage (SDC-064..066) — Phase 9 ───────────────────────
# These fire ONLY when design context is supplied. They use STRUCTURAL
# evidence (which instance pins a port's net connects to) rather than names,
# and they distinguish COVERAGE (was it constrained?) from CORRECTNESS (was it
# constrained correctly?) — a covered port can still violate other rules.

_r("SDC-064", "warning", "Unconstrained Data Input (structural)",
   "An input port whose net structurally drives instance data pins (D/DIN/DATA) has no set_input_delay.",
   "The port is structurally a data boundary, so its external setup/hold is genuinely unconstrained — this is stronger evidence than a name-based guess.",
   "Add set_input_delay (-max/-min) for the port, or verify the port is intentionally exempt.",
   "", "design_coverage", "1.4.0")

_r("SDC-065", "warning", "Unconstrained Data Output (structural)",
   "An output port whose net is structurally driven by instance data pins (Q/DOUT) has no set_output_delay.",
   "The port is structurally a data output boundary, so its external setup/hold is genuinely unconstrained.",
   "Add set_output_delay (-max/-min) for the port, or verify the port is intentionally exempt.",
   "", "design_coverage", "1.4.0")

_r("SDC-066", "warning", "Partially Constrained Bus",
   "A bus port has I/O delay on only some of its bits (provable from the SDC collections), leaving the other bits unconstrained.",
   "Partial bus coverage means some bits of the interface have no external timing constraint — a subtle gap that full-interface constraint reviews exist to catch.",
   "Constrain the whole bus (e.g. [get_ports data_in]) or explicitly constrain every bit.",
   "", "design_coverage", "1.4.0")


# ── Constraint interactions (SDC-067..070) — Phase 10 ────────────────────────
# Semantic interaction analysis: how multiple individually-valid constraints
# interact. Only provable conditions are reported; everything else is an
# interaction status, never an error. Dual-line provenance (line/line2).

_r("SDC-067", "info", "Exact Duplicate Constraint",
   "The same normalized constraint (same command, objects, options, value) is issued more than once.",
   "The second command is a no-op — the constraint already exists. Harmless but redundant; often a copy-paste artifact.",
   "Remove the duplicate, or keep it only if a comment explains why it is intentional.",
...
_r("SDC-069", "warning", "Contradictory Max/Min Delay",
   "set_max_delay value is smaller than set_min_delay on provably identical endpoints.",
   "A timing window where max < min is impossible: no path can satisfy both required_max < required_min. This is provable without STA.",
   "Fix the values so set_max_delay >= set_min_delay on the same endpoints.",
   "", "constraint_interactions", "1.4.0")

_r("SDC-070", "info", "Timing Exception Interaction",
   "A set_false_path and a set_multicycle_path/set_max_delay/set_min_delay reference overlapping endpoint objects.",
   "Object overlap is NOT proof that the same timing path is affected — the exact interaction requires STA/path analysis. A false path may legally coexist with an exception on different edge/phase combinations.",
   "Review the exceptions with STA to confirm whether the same path is both false and constrained.",
   "", "constraint_interactions", "1.4.0")
