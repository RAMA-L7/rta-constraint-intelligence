...
| 8 | Legacy | `app.py`, `.streamlit/`, remaining `ui/**` (20 modules) → `legacy/streamlit/`; `svg/` → `legacy/assets/`; `graphify-out/` → `legacy/experiments/` |
...
2. **`ui/` shim package** — `ui/__init__.py` + 20 module shims alias the old
   Streamlit UI into `legacy/streamlit/ui/`, and `ui/theme`/`ui/feedback` into
   `rta/branding/tokens/` and `rta/workspace/server/` respectively.
...
- `[tool.setuptools.packages.find]` now ships `rta*` + `ui*`; `legacy*` is
  **explicitly excluded** from the wheel (preserved in-repo only).
