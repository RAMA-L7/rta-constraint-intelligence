/* ═══════════════════════════════════════════════════════════════════════════
   Ṛta — components.js
   Product component builders. ALL user-controlled values are escaped via
   theme.esc (Phase 14/17 stored-XSS discipline).
   ═══════════════════════════════════════════════════════════════════════════ */
...
/* ── Empty states — answer "what does this mean" + "what next" ──────────── */
...
/* ── Typed error states (never one red box, never raw traces) ───────────── */
...
/* ── Source viewer with highlights + dual-line markers ──────────────────── */
...
export function table(headers, rows, { mono = true, clickable = false, selectedKey = null } = {}) {
  ...
  const trs = rows.map(r => {
    const click = clickable ? ` class="row-click${r.key && r.key === selectedKey ? " row-selected" : ""}" data-key="${escAttr(r.key || "")}"` : "";
...
export function findingDetailHtml(it, rule) {
  ...
  if (it.clk) meta.push(["Clock", it.clk]);
  ...
  if (rule) {
    ...
    if (rule.fix) parts.push(`<div class="insp-section"><div class="insp-k">Fix</div><div class="insp-v mono">${esc(rule.fix)}</div></div>`);
  }
  if (it.requires_sta) {
    parts.push(`<div class="callout co-warning" style="margin-top:6px"><span><strong>REQUIRES STA / PATH ANALYSIS</strong> — this overlap cannot be proven or disproven by structural analysis alone.</span></div>`);
  }
  return parts.join("");
}
