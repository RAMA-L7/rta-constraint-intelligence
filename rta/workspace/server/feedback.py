...
The data layer (FeedbackEntry / load_feedback / save_feedback) is pure-stdlib
so the local API server can record feedback without Streamlit. The Streamlit
widget + dashboard import their UI dependencies lazily inside the functions.
"""
...
SAMPLE_SDCS = {
    "✅ Real Design (RISC-V Core)": "rta/examples/samples/real_design_full.sdc",
    "🐛 Buggy — Missing Clocks": "rta/examples/samples/buggy_no_clocks.sdc",
    "⚠️ Warnings — Clock Groups": "rta/examples/samples/warning_heavy.sdc",
    "📝 Malformed SDC": "rta/examples/samples/edge_case_malformed.sdc",
    "📭 Empty SDC": "rta/examples/samples/edge_case_empty.sdc",
    "🔬 Edge Values": "rta/examples/samples/edge_case_extreme_values.sdc",
}
