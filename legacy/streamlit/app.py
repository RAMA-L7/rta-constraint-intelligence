"""
Ṛta — Constraint Intelligence Platform
Deterministic checking, generation, and multi-corner management for
...
# ── Netlist (optional) — shared helper for Checker / Coverage / Clock tabs ──────
# Block-level scope: a single flat RTL or gate-level Verilog module. Full-chip
# (hierarchical, multi-module) resolution is a planned follow-up on top of this
# same DesignContext model — see design_context.py docstring.
def _netlist_upload_widget(key_prefix: str):
    """Optional RTL or gate-level Verilog upload for design-aware cross-checks.

    Returns a parsed DesignContext, or None if nothing was uploaded/pasted.
    Purely additive — every tab's existing SDC-only flow is unchanged when
    this returns None.
    """
    with st.expander("🧬 Netlist (optional) — cross-check against your real design", expanded=False):
        st.caption(
            "Upload an RTL or gate-level (post-synthesis) Verilog file — block-level "
            "(single flat module). Ṛta will resolve every get_ports/get_pins/get_cells "
            "target against the real design and flag ports with no delay constraint. "
            "Best-effort structural read — not a full Verilog elaborator."
        )
        nl_uploaded = st.file_uploader(
            "Upload .v (RTL or gate-level)", type=["v", "vh"], key=f"{key_prefix}_netlist_upload"
        )
        nl_pasted = st.text_area(
            "Or paste Verilog here", height=100, key=f"{key_prefix}_netlist_paste",
            placeholder="module my_block (input clk, input rst_n, ... );\n...\nendmodule",
        )
        nl_text = None
        if nl_uploaded:
            nl_text = nl_uploaded.read().decode("utf-8", errors="replace")
        elif nl_pasted.strip():
            nl_text = nl_pasted

        if not nl_text:
            return None

        outcome = parse_verilog(nl_text)
        ctx = outcome.context
        if outcome.warnings:
        ...
        if outcome.errors:
            for e in outcome.errors:
                st.error(f"❌ {e}")
        if not ctx or not ctx.top_module:
            st.error("Could not parse a module from this file — netlist cross-checks skipped.")
            return None

        st.success(
            f"Parsed module `{ctx.top_module}` — "
...
def _render_netlist_cross_checks(findings, ctx):
    """Render the design-aware reference-resolution section (SDC-055/056/057/059).

    Called only when a netlist/DesignContext was supplied.
    """
    st.divider()
    st.subheader(f"🧬 Netlist Cross-Checks ({len(findings)})")
    errors = [f for f in findings if f.sev == "error"]
    warnings = [f for f in findings if f.sev == "warning"]

    ncols = st.columns(3)
    ncols[0].metric("Ports in netlist", len(ctx.ports))
    ncols[1].metric("❌ Errors", len(errors))
    ncols[2].metric("⚠️ Warnings", len(warnings))

    if not findings:
        st.success("✅ Every SDC target resolved against a real object in the netlist.")
    else:
        for f in findings:
            icon = {"error": "🔴", "warning": "🟡", "info": "🔵"}.get(f.sev, "⚪")
            with st.expander(f"{icon} `{f.code}` — {f.msg[:90]}{'…' if len(f.msg) > 90 else ''}"):
                badge_cls = {"error": "err-badge", "warning": "warn-badge", "info": "info-badge"}.get(f.sev, "info-badge")
                st.markdown(f'<span class="{badge_cls}">{f.sev.title()}</span>', unsafe_allow_html=True)
                st.write(f.msg)
                if f.line:
...
if st.session_state.app_view == "test_drive":
    ...
    render_test_drive()
    st.stop()
elif st.session_state.app_view == "feedback":
    from ui.feedback import render_dashboard
    render_dashboard()
    st.stop()
...
with tab_checker:
    ...
    st.caption("Deterministic constraint validation — upload your .sdc file or paste the content below.")
    ...
    checker_netlist = _netlist_upload_widget("checker")

    if sdc_text:
        if st.button("▶ Run Check", type="primary", use_container_width=True, key="run_check"):
            # ── Custom Rules upload ───────────────────────────────────────────────
            custom_rule_results = []
        ...
        # ── Info ─────────────────────────────────────────────────────────────
            ...
            # ── Netlist Cross-Checks (only if a netlist was supplied) ──────────────
            if checker_netlist is not None:
                net_findings = validate_design_references(sdc_text, checker_netlist)
                _render_netlist_cross_checks(net_findings, checker_netlist)
...
with tab_generator:
    ...
    st.caption("Compose a signoff-ready SDC file from a structured, deterministic spec.")
...
with tab_clock_rel:
    ...
    cr_netlist = _netlist_upload_widget("clockrel")
    ...
    if cr_text and st.button("🔍 Analyze Clock Relations", type="primary", use_container_width=True, key="cr_analyze_btn"):
        ...
        if cr_netlist is not None:
            cr_net_findings = validate_design_references(cr_text, cr_netlist)
            # Reference-resolution errors on lines containing a clock definition
            # are the ones that matter most here (a broken clock source port).
            cr_text_lines = cr_text.splitlines()
            clock_src_issues = [
                f for f in cr_net_findings
                if f.line and 0 < f.line <= len(cr_text_lines)
                and "create_" in cr_text_lines[f.line - 1] and "clock" in cr_text_lines[f.line - 1]
            ]
            if clock_src_issues:
                st.error(f"🧬 Netlist check: {len(clock_src_issues)} clock source port(s) not found in the netlist.")
                for f in clock_src_issues:
                    st.write(f"🔴 {f.msg}")
            elif cr_net_findings:
                st.warning(f"🧬 Netlist check: {len(cr_net_findings)} other design-reference issue(s) found — see Checker tab for full detail.")
            else:
                st.success("🧬 Netlist check: every clock source port matched a real port in the netlist.")
...
with tab_coverage:
    ...
    cv_netlist = _netlist_upload_widget("coverage")
    ...
    if cv_text and st.button("📊 Analyze Coverage", type="primary", use_container_width=True, key="cv_analyze_btn"):
        with st.spinner("Analyzing constraint coverage..."):
            ...
            cv_design_cov = analyze_coverage(cv_text, cv_netlist) if cv_netlist is not None else None
        ...
        if not cv_result.categories:
            ...
            # ── Real-Design Port Coverage (only if a netlist was supplied) ──
            if cv_design_cov is not None:
                st.divider()
                st.subheader("🧬 Real-Design Port Coverage")
                st.caption(
                    "Coverage against the actual ports in your netlist, with structural evidence "
                    "per port — catches unconstrained real pins that a syntax-only pass can't see, "
                    "and avoids false positives on ports the design shows are clocks/resets by "
                    "how they actually connect."
                )
                all_ports = cv_design_cov.inputs + cv_design_cov.outputs
                constrained_n = sum(1 for p in all_ports if p.status == "CONSTRAINED")
                exempt_n = sum(1 for p in all_ports if p.status == "EXEMPT")
                unconstrained = [p for p in all_ports if p.status == "UNCONSTRAINED"]
                unknown = [p for p in all_ports if p.status == "UNKNOWN"]
                scoreable = len(all_ports) - exempt_n
                real_score = ((constrained_n) / scoreable * 100) if scoreable else 100.0

                rc1, rc2, rc3, rc4 = st.columns(4)
                rc1.metric("Real I/O Coverage", f"{real_score:.0f}%")
                rc2.metric("Ports in netlist", len(all_ports))
                rc3.metric("Unconstrained", len(unconstrained))
                rc4.metric("Unknown / ambiguous", len(unknown))

                if unconstrained:
                    with st.expander(f"⚠️ Unconstrained real ports ({len(unconstrained)})", expanded=True):
                        for p in unconstrained:
                            st.markdown(f"**{p.name}** ({p.direction}, {p.port_class}) — {p.evidence}")
                if unknown:
                    with st.expander(f"🔵 Unknown / ambiguous ports ({len(unknown)})"):
                        st.caption("No structural or naming evidence either way — reviewed manually, not guessed.")
                        for p in unknown:
                            st.markdown(f"**{p.name}** ({p.direction}) — {p.evidence}")
                if not unconstrained and not unknown:
                    st.success("✅ Every real I/O port in the netlist is constrained or structurally exempt.")

                if cv_design_cov.clocks:
                    with st.expander(f"🕐 Clock coverage ({len(cv_design_cov.clocks)})"):
                        for c in cv_design_cov.clocks:
                            resolved = "✅ resolved" if c.structurally_resolved else "⚠️ unresolved target"
                            st.markdown(f"**{c.name}** — period {c.period}ns, {resolved}, status: {c.status}")
...
# ════════════════════════════════════════════════════════════════════════════
# CONSTRAINT INTERACTIONS TAB
# ════════════════════════════════════════════════════════════════════════════
with tab_interactions:
    st.subheader("🔗 Constraint Interactions")
    st.caption(
        "Finds duplicate, overriding, and contradictory constraints within a single SDC — "
        "the bugs that live *between* commands, not inside any one of them. Block-level scope."
    )

    ia_col_up, ia_col_paste = st.columns([1, 2])
    with ia_col_up:
        ia_uploaded = st.file_uploader("Upload .sdc / .tcl / .txt", type=["sdc", "tcl", "txt"], key="ia_upload")
    with ia_col_paste:
        ia_pasted = st.text_area(
            "Or paste SDC text here", height=140,
            placeholder=(
                "create_clock -name clk -period 5.0 [get_ports clk]\n"
                "set_clock_uncertainty -setup 0.15 [get_clocks clk]\n"
                "set_clock_uncertainty -setup 0.15 [get_clocks clk]\n"
                "set_max_delay 2.0 -from [get_ports data_in]\n"
                "set_min_delay 3.0 -from [get_ports data_in]\n"
            ),
            key="ia_paste",
        )
    ...
    ia_text = None
    if ia_uploaded:
        ia_text = ia_uploaded.read().decode("utf-8", errors="replace")
    elif ia_pasted.strip():
        ia_text = ia_pasted

    ia_netlist = _netlist_upload_widget("interactions")

    if ia_text and st.button("🔗 Analyze Interactions", type="primary", use_container_width=True, key="ia_analyze_btn"):
        with st.spinner("Analyzing constraint interactions..."):
            ia_result = analyze_interactions(ia_text, ia_netlist)

        st.divider()
        cats = {}
        for f in ia_result.findings:
            cats.setdefault(f["category"], []).append(f)

        ic1, ic2, ic3, ic4 = st.columns(4)
        ic1.metric("Total findings", len(ia_result.findings))
        ic2.metric("🔴 Conflicts", len(cats.get("DEFINITE_CONFLICT", [])) + len(cats.get("POSSIBLE_CONFLICT", [])))
        ...
        if not ia_result.findings:
            ...
            for f in ia_result.findings:
                ...
                with st.expander(f"{icon} `{f['code']}` — {f['category'].replace('_', ' ').title()} (lines {f['line2']}↔{f['line']})"):
                    badge_cls = {"error": "err-badge", "warning": "warn-badge", "info": "info-badge"}.get(f["severity"], "info-badge")
                    st.markdown(f'<span class="{badge_cls}">{f["severity"].title()}</span>', unsafe_allow_html=True)
                    st.write(f["msg"])
                    st.caption(f"Command: `{f['command']}` · Confidence: {f['confidence']}")

        if ia_result.legal_multiples:
            with st.expander(f"ℹ️ Legal multiple-constraint patterns detected ({len(ia_result.legal_multiples)})"):
                st.caption("Multiple constraints on the same object that are valid by SDC semantics (e.g. -add clocks) — not flagged as findings.")
                for lm in ia_result.legal_multiples:
                    st.write(lm)
    elif not ia_text:
        st.info("Upload an SDC file or paste text above to analyze constraint interactions.")
...
# ════════════════════════════════════════════════════════════════════════════
with tab_readiness:
    st.subheader("✅ Constraint Readiness")
    st.caption(
        "Aggregates the Checker's own findings into a signoff-readiness verdict across 7 "
        "dimensions (Clocks, I/O, Exceptions, Coverage, Consistency, Analysis Trust, Design Context). "
        "READY ≠ STA signoff — this is a pre-STA quality gate, not a timing tool."
    )

    rd_col_up, rd_col_paste = st.columns([1, 2])
    with rd_col_up:
        rd_uploaded = st.file_uploader("Upload .sdc / .tcl / .txt", type=["sdc", "tcl", "txt"], key="rd_upload")
    with rd_col_paste:
        rd_pasted = st.text_area(
            "Or paste SDC text here", height=140,
            placeholder=(
                "set sdc_version 2.2\n"
                "create_clock -name clk -period 5.0 [get_ports clk]\n"
    ...
    rd_text = None
    if rd_uploaded:
        rd_text = rd_uploaded.read().decode("utf-8", errors="replace")
    elif rd_pasted.strip():
        rd_text = rd_pasted

    if rd_text and st.button("✅ Assess Readiness", type="primary", use_container_width=True, key="rd_analyze_btn"):
        with st.spinner("Aggregating readiness evidence..."):
            rd_check_result = check_sdc(rd_text)
            rd_result = analyze_readiness(rd_check_result)

        st.divider()

        verdict_style = {
            "READY": ("success", "✅"),
            "READY_WITH_ADVISORIES": ("success", "✅"),
            "REVIEW_REQUIRED": ("warning", "🟡"),
            "BLOCKED": ("error", "🔴"),
            "INSUFFICIENT_CONTEXT": ("info", "🔵"),
        }
        style, icon = verdict_style.get(rd_result.overall, ("info", "⚪"))
        getattr(st, style)(f"{icon} **Overall: {rd_result.overall.replace('_', ' ').title()}** · mode: {rd_result.mode}")
        ...
        for dim_name, evidence in rd_result.dimensions.items():
            dim_icon = {
                "READY": "✅", "READY_WITH_ADVISORIES": "✅", "REVIEW_REQUIRED": "🟡",
                "BLOCKED": "🔴", "INSUFFICIENT_CONTEXT": "🔵", "NOT_APPLICABLE": "⚪",
            }.get(evidence.status, "⚪")
            with st.expander(f"{dim_icon} {dim_name} — {evidence.status.replace('_', ' ').title()}"):
                if evidence.summary:
                    st.caption(evidence.summary)
                for f in evidence.findings:
                    sev_icon = {"error": "🔴", "warning": "🟡", "info": "🔵"}.get(f.severity, "⚪")
                    st.markdown(f"{sev_icon} `{f.code}` — {f.msg}")
                    if f.action:
                        st.caption(f"Suggested action: {f.action} (priority {f.priority})")

        if rd_result.actions:
            st.subheader("🎯 Prioritized Actions")
            for a in rd_result.actions:
                st.markdown(f"**[{a['priority']}] {a['category'].replace('_', ' ').title()}** — {a['count']} finding(s): {', '.join(a['evidence'])}")

        if rd_result.notes:
            with st.expander("📝 Notes"):
                for n in rd_result.notes:
                    st.write(n)
    elif not rd_text:
        st.info("Upload an SDC file or paste text above to assess constraint readiness.")
...
# ════════════════════════════════════════════════════════════════════════════
with tab_linter:
    from ui.tab_linter import render as render_linter
    render_linter()
...
# ════════════════════════════════════════════════════════════════════════════
with tab_converter:
    from ui.tab_converter import render as render_converter
    render_converter()
...
# ════════════════════════════════════════════════════════════════════════════
with tab_rules_ref:
    from ui.tab_rules import render as render_rules
    render_rules()
